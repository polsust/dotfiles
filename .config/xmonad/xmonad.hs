...
-- Apps
    ...
    -- launch rofi drun
    , ((modm,               xK_e    ), spawn "rofi -show drun")
    ...
    -- run gpick
    , ((modm .|. shiftMask, xK_x    ), spawn "gpick -p")
...
-- End XF86 Controls
    ...
    , ((altm,               xK_Tab   ), nextMatch Backward (return True))
 ...
 -- , ((modm              , xK_b     ), sendMessage ToggleStruts)
  ...
  -- Quit xmonad
    ...
    --
    --[((m .|. modm .|. shiftMask, key), screenWorkspace sc >>= flip whenJust (windows . f))
...
myLayout = spacingRaw False (Border 0 10 0 10) True (Border 10 0 10 0) True $ tiled ||| Mirror tiled ||| Full
  where
...
myManageHook = composeAll
    ...
    -- Spawn windows after the focused one
    <+> doF W.swapDown
